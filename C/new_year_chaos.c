...
void minimumBribes(int q_count, int* q) {
    ...
    for(int i = 0; i < q_count;i++){
        ...
        // Check if the current person moved more than 2 positions ahead
        int bribes = q[i] - (i+1);
        bribes = (bribes > 0)?bribes:0;
        if(bribes > 2)
        {
            printf("Too chaotic\n");
            return;
        ...
        {
            // Loop through people ahead of current, see if they bribed current person
            int bound = MAX(0, q[i]-2);
